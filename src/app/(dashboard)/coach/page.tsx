...
function CoachDashboardSkeleton() {
    return (
        <div className="space-y-8 animate-pulse mt-8">
            <div className="bg-white h-48 rounded-2xl border border-slate-100 p-6"></div>
            <div className="bg-white h-64 rounded-2xl border border-slate-100 p-6"></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white h-32 rounded-2xl border border-slate-100 p-6"></div>
                <div className="bg-white h-32 rounded-2xl border border-slate-100 p-6"></div>
                <div className="bg-white h-32 rounded-2xl border border-slate-100 p-6"></div>
            </div>
        </div>
...
async function CoachDashboardContent({ userId, profile }: { userId: string, profile: any }) {
    const supabase = await createClient()
    ...
    if (assignedIds.length > 0) {
        lessonsQuery = lessonsQuery.or(`coach_id.eq.${userId},student_id.in.(${assignedIds.join(',')})`)
    } else {
        lessonsQuery = lessonsQuery.eq('coach_id', userId)
    }
    ...
    const currentRate = isAdmin ? 1.0 : calculateCoachRate(userId, allLessons as any || [], today, profile?.override_coach_rank)
    ...
    const thisMonthStats = calculateMonthlyStats(userId, thisMonthLessons as any[], currentRate)

    // Calculate Last Month Stats for KPI Diff
    const lastMonthDate = subMonths(today, 1)
    const lastMonthStart = startOfMonth(lastMonthDate)
    const lastMonthEnd = endOfMonth(lastMonthDate)
    const lastMonthRate = isAdmin ? 1.0 : calculateCoachRate(userId, allLessons as any || [], lastMonthDate, profile?.override_coach_rank)
    const lastMonthLessons = allLessons?.filter(l => {
        const d = new Date(l.lesson_date)
        return d >= lastMonthStart && d <= lastMonthEnd
    }) || []
    const lastMonthStats = calculateMonthlyStats(userId, lastMonthLessons as any[], lastMonthRate)
    ...
    return (
        <div className="space-y-8 animate-fade-in-up">
            ...
            <TodayScheduleWidget coachId={userId} />
